...
class SpecialGlobalCheckUser extends SpecialPage {
	...
	public function __construct() {
		parent::__construct( 'GlobalCheckUser', 'globalcheckuser' );
	}
	...
	public function execute( $subpage ) {
		...
		if ( $this->getUser()->isAllowed( 'globalcheckuser-log' ) ) {
			$subtitleLink = $this->getLinkRenderer()->makeKnownLink(
				SpecialPage::getTitleFor( 'GlobalCheckUserLog' ),
				$this->msg( 'globalcheckuser-showlog' )->text()
			);
			if ( !$user === false ) {
				$subtitleLink .= ' | ' . $this->getLinkRenderer()->makeKnownLink(
					SpecialPage::getTitleFor( 'GlobalCheckUserLog', $user ),
					$this->msg( 'globalcheckuser-recent-checks' )->text()
				);
			}
	...
	protected function showIntroductoryText() {
		$cidrLimit = $this->getConfig()->get( 'GlobalCheckUserCIDRLimit' );
		$this->getOutput()->addWikiMsg(
			'globalcheckuser-summary',
			$cidrLimit['IPv4'],
			$cidrLimit['IPv6']
		);
	}
	...
	protected function showForm( $user, $reason, $checktype, $ip, $xff, $name, $period ) {
		...
		$form = Xml::openElement( 'form', [ 'action' => $action,
			'name' => 'globalcheckuserform', 'id' => 'globalcheckuserform', 'method' => 'post' ] );
		$form .= '<fieldset><legend>' . $this->msg( 'globalcheckuser-query' )->escaped() . '</legend>';
		$form .= Xml::openElement( 'table', [ 'style' => 'border:0' ] );
		$form .= '<tr>';
		$form .= '<td>' . $this->msg( 'globalcheckuser-target' )->escaped() . '</td>';
		...
		$form .= ' ' . Xml::label( $this->msg( 'globalcheckuser-ips' )->text(), 'subuserips' ) . '</td>';
		$form .= '<td>' .
			Xml::radio( 'checktype', 'subedits', $encedits, [ 'id' => 'subedits' ] );
		$form .= ' ' . Xml::label( $this->msg( 'globalcheckuser-edits' )->text(), 'subedits' ) . '</td>';
		$form .= '<td>' .
			Xml::radio( 'checktype', 'subipusers', $encipusers, [ 'id' => 'subipusers' ] );
		$form .= ' ' .
			Xml::label( $this->msg( 'globalcheckuser-users' )->text(), 'subipusers' ) . '</td>';
		$form .= '</tr>';
		$form .= Xml::closeElement( 'table' );
		$form .= Xml::closeElement( 'td' );
		$form .= '</tr><tr>';
		$form .= '<td>' . $this->msg( 'globalcheckuser-reason' )->escaped() . '</td>';
		$form .= '<td>' . Xml::input( 'reason', 46, $reason,
			[ 'maxlength' => '150', 'id' => 'checkreason' ] );
		$form .= '&#160; &#160;' . Xml::submitButton( $this->msg( 'globalcheckuser-check' )->text(),
	...
	protected function getPeriodMenu( $selected ) {
		$s = '<label for="period">' .
			$this->msg( 'globalcheckuser-period' )->escaped() . '</label>&#160;';
		$s .= Xml::openElement(
			'select',
			[ 'name' => 'period', 'id' => 'period', 'style' => 'margin-top:.2em;' ]
		);
		$s .= Xml::option( $this->msg( 'globalcheckuser-week-1' )->text(), 7, $selected === 7 );
		$s .= Xml::option( $this->msg( 'globalcheckuser-week-2' )->text(), 14, $selected === 14 );
		$s .= Xml::option( $this->msg( 'globalcheckuser-month' )->text(), 31, $selected === 31 );
		$s .= Xml::option( $this->msg( 'globalcheckuser-all' )->text(), 0, $selected === 0 );
	...
	protected function addJsCIDRForm() {
		$s = '<fieldset id="mw-checkuser-cidrform" style="display:none; clear:both;">' .
			'<legend>' . $this->msg( 'globalcheckuser-cidr-label' )->escaped() . '</legend>';
		$s .= '<textarea id="mw-checkuser-iplist" dir="ltr" rows="5" cols="50"></textarea><br />';
		$s .= $this->msg( 'globalcheckuser-cidr-res' )->escaped() . '&#160;' .
	...
	protected function checkReason( $reason ) {
		return ( !$this->getConfig()->get( 'GlobalCheckUserForceSummary' ) || strlen( $reason ) );
	}
	...
	protected function doMassUserBlock( $users, $blockParams, $tag = '', $talkTag = '' ) {
		...
		if ( !$this->getUser()->isAllowed( 'block' ) || $this->getUser()->isBlocked()
			...
			$this->getOutput()->addWikiMsg( 'globalcheckuser-block-failure' );
			return;
		} elseif ( $usersCount > $this->getConfig()->get( 'GlobalCheckUserMaxBlocks' ) ) {
			$this->getOutput()->addWikiMsg( 'globalcheckuser-block-limit' );
			return;
		} elseif ( !$blockParams['reason'] ) {
			$this->getOutput()->addWikiMsg( 'globalcheckuser-block-noreason' );
			return;
		}
		...
		if ( $blockedCount > 0 ) {
			...
			$this->getOutput()->addWikiMsg( 'globalcheckuser-block-success',
	...
	protected function noMatchesMessage( $userName, $checkLast = true ) {
		if ( $checkLast ) {
			...
			if ( $lastEdit ) {
				...
				return $this->msg( 'globalcheckuser-nomatch-edits',
					$lang->date( $lastEditTime, true ),
					$lang->time( $lastEditTime, true )
				)->parseAsBlock();
			}
		}
		return $this->msg( 'globalcheckuser-nomatch' )->parseAsBlock();
	}
	...
	protected function doUserIPsRequest( $user, $reason = '', $period = 0 ) {
		...
		$ret = $dbr->select(
			...
			[
				...
				'gcuc_ip_hex',
				'COUNT(*) AS count',
				'MIN(gcuc_timestamp) AS first',
				'MAX(gcuc_timestamp) AS last',
			],
			[ 'gcuc_user' => $user_id, $time_conds ],
			__METHOD__,
			[
				'ORDER BY' => 'last DESC',
		...
		if ( !$dbr->numRows( $ret ) ) {
			...
			foreach ( $ret as $row ) {
				if ( $counter >= 5000 ) {
					$out->addWikiMsg( 'globalcheckuser-limited' );
					break;
				}
				$ips_edits[$row->gcuc_ip] = $row->count;
				$ips_first[$row->gcuc_ip] = $row->first;
				$ips_last[$row->gcuc_ip] = $row->last;
				$ips_hex[$row->gcuc_ip] = $row->cuc_ip_hex;
				++$counter;
			}
			...
			foreach ( $ips_edits as $ip => $edits ) {
				...
				$ipedits = $dbr->estimateRowCount( 'gcu_changes', '*',
					[ 'gcuc_ip_hex' => $ips_hex[$ip], $time_conds ],
					__METHOD__ );
				...
				if ( $ipedits > $ips_edits[$ip] ) {
					$s .= ' <i>(' .
						$this->msg( 'globalcheckuser-ipeditcount' )->numParams( $ipedits )->escaped() .
						')</i>';
				}
				...
				$s .= '<small>' . $this->msg( 'globalcheckuser-toollinks', urlencode( $ip ) )->parse() .
					'</small>';
				$s .= '</div>';
				$s .= "</li>\n";
			}
	...
	protected function getBlockFlag( Block $block ) {
		if ( $block->getType() == Block::TYPE_AUTO ) {
			...
			$ret = $this->getLinkRenderer()->makeKnownLink(
				...
				$this->msg( 'globalcheckuser-blocked' )->text(),
				[],
				[
					'type' => 'block',
	...
	protected function doIPEditsRequest( $ip, $xfor = false, $reason = '', $period = 0 ) {
		...
		$index = $xfor ? 'gcuc_xff_hex_time' : 'gcuc_ip_hex_time';
		if ( strpos( $ip, '/' ) !== false ) {
			// Quick index check only OK if no time constraint
			if ( $period ) {
				$rangecount = $dbr->selectField( 'gcu_changes', 'COUNT(*)',
					[ $ip_conds, $time_conds ],
					__METHOD__,
					[ 'USE INDEX' => $index ] );
			} else {
				$rangecount = $dbr->estimateRowCount( 'gcu_changes', '*',
					[ $ip_conds ],
					__METHOD__,
					[ 'USE INDEX' => $index ] );
			}
		...
		if ( isset( $rangecount ) && $rangecount > 5000 ) {
			...
			$ret = $dbr->select(
				...
				__METHOD__,
				[
					'GROUP BY' => 'gcuc_ip_hex',
					'ORDER BY' => 'gcuc_ip_hex',
					'LIMIT' => 5001,
					'USE INDEX' => $index,
				]
			);
			// List out each IP that has edits
			$s = $this->msg( 'globalcheckuser-too-many' )->parseAsBlock();
			$s .= '<ol>';
			foreach ( $ret as $row ) {
				if ( $counter >= 5000 ) {
					$out->addWikiMsg( 'globalcheckuser-limited' );
		...
		$ret = $dbr->select(
			'gcu_changes',
			[
				'gcuc_namespace', 'gcuc_title', 'gcuc_user', 'gcuc_user_text', 'gcuc_comment',
				'gcuc_actiontext', 'gcuc_timestamp', 'gcuc_minor', 'gcuc_page_id', 'gcuc_type',
				'gcuc_this_oldid', 'gcuc_last_oldid', 'gcuc_ip', 'gcuc_xff', 'gcuc_agent'
			],
			[ $ip_conds, $time_conds ],
			__METHOD__,
			[
		...
		if ( !$dbr->numRows( $ret ) ) {
			...
			$lb = new LinkBatch;
			foreach ( $ret as $row ) {
				$userText = str_replace( ' ', '_', $row->gcuc_user_text );
				if ( $row->gcuc_title !== '' ) {
					$lb->add( $row->gcuc_namespace, $row->gcuc_title );
				}
